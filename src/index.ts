...
import { detectLanguage, detectPiiEntities, redactText } from "./comprehend.js";
import {
...
import { checkTextSize, filterEntities, getLanguageName } from "./utils.js";
...
// Register detect_language tool
server.registerTool(
  "detect_language",
  {
    description:
      "Detect the dominant language(s) in text. Returns language code (e.g., 'en', 'es'), human-readable name, and confidence score. Useful for understanding what language content is written in before processing.",
    inputSchema: DetectLanguageParamsSchema,
  },
  async (params) => {
    try {
      ...
      const result: DetectedLanguage[] = languages.map((lang) => ({
        ...
        language_name: getLanguageName(lang.LanguageCode ?? "unknown"),
